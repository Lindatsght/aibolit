...
define('PASS', '111524'); // пароль для запуска
...
function WarningPHP($l_FN, $l_Content, &$par_Pos)
{
  ...
  if (!$l_Res) {
    if (preg_match('/(include|include_once|require|require_once).+?http:\/\/.+?\.txt.+?;/smi', $l_Content, $l_Found, PREG_OFFSET_CAPTURE)) {
       $l_Res = true;
       $par_Pos = $l_Found[2];
    }
  }
